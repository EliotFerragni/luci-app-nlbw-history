...
architecture independent, so the same file works on any target:
    ...
    ssh root@192.168.1.1 'apk add --allow-untrusted /tmp/luci-app-nlbw-history-1.1.0-r1.apk'

    # 24.10 and older
    scp luci-app-nlbw-history_1.1.0-1_all.ipk root@192.168.1.1:/tmp/
    ssh root@192.168.1.1 'opkg install /tmp/luci-app-nlbw-history_1.1.0-1_all.ipk'
...
**Full diagnostics** in Settings shows everything both samplers report, which
is the same as running `nlbw-history-collect --status` and
`nlbw-history-live --status` on the router.
